...
function renderWorkoutsPage(searchTerm = '', difficultyFilter = 'All') {
        ...
        if (filteredWorkouts.length > 0) {
            ...
            workoutsHtml += `
                <div class="col-md-6 col-lg-4 mb-4" >
                    <div class="card workout-card ${isCompleted ? 'border-success' : ''}" data-bs-toggle="modal" data-bs-target="#exerciseDetailModal" data-id="${workout.id}" style="cursor: pointer;" data-loggable="true" data-workout-id="${workout.id}">
    ...
    if (workoutCount === 0) {
        categoriesHtml = `<div class="alert alert-warning text-center">No workouts found matching your criteria.</div>`;
    }
    ...
    return `
        ...
        <div class="row mb-4 filter-controls">
            ...
            <div class="col-lg-6 d-flex">
                ...
                <button class="btn btn-outline-danger" id="clear-filters-btn" title="Clear Filters"><i class="fas fa-times"></i></button>
            </div>
        </div>
                ...
                <div class="card-body">
                    <div class="row text-center">
                        <div class="col-md-4 mb-3">
                            <div class="workout-badge bg-warning">
                                <i class="fas fa-fire"></i>
                            </div>
                            <h5>7-Day Streak</h5>
                            <p>Complete workouts for 7 consecutive days</p>
                            <span class="badge ${currentUser.streak >= 7 ? 'bg-success' : 'bg-secondary'}">
                                ${currentUser.streak || 0}/7 days
                            </span>
                        </div>
                        <div class="col-md-4 mb-3">
                            <div class="workout-badge bg-info">
                                <i class="fas fa-dumbbell"></i>
                            </div>
                            <h5>Strength Master</h5>
                            <p>Complete all strength workouts</p>
                            <span class="badge bg-secondary">0/3 completed</span>
                        </div>
                        <div class="col-md-4 mb-3">
                            <div class="workout-badge bg-danger">
...
function renderProgressPage() {
            ...
            <div class="col-md-4 mb-4">
                <div class="card h-100 achievement-card ${highestTierUnlocked ? 'unlocked' : ''}">
                    <div class="card-body text-center">
                        ...
                        ${highestTierUnlocked ? `<p class="tier-unlocked">${highestTierUnlocked.name} Unlocked!</p><small>${highestTierUnlocked.description}</small>` : `<p class="text-muted">Keep going!</p>`}
                        ${nextTier ? `<div class="progress mt-2" style="height: 5px;"><div class="progress-bar" role="progressbar" style="width: ${Math.round((userValue / nextTier.threshold) * 100)}%;" aria-valuenow="${userValue}" aria-valuemin="0" aria-valuemax="${nextTier.threshold}"></div></div><small class="text-muted">Next: ${nextTier.name} (${nextTier.description})</small>` : '<p class="text-success mt-2">All tiers unlocked!</p>'}
                    </div>
                </div>
            </div>
        `;
    ...
    return `
        ...
        <div class="row mb-5">
            ...
            <div class="col-md-3 text-center">
                <h1 class="display-4 text-success">${currentUser.streak || 0}</h1>
                <h5>Day Streak</h5>
            </div>
            <div class="col-md-3 text-center">
                <h1 class="display-4 text-info">${totalCalories}</h1>
        ...
        userNotifications.forEach(notification => {
            notificationsHtml += `
                <div class="d-flex align-items-center p-3 border-bottom">
                    <div class="me-3 text-${notification.type === 'success' ? 'success' : notification.type === 'warning' ? 'warning' : 'info'}">
                        <i class="fas fa-${notification.type === 'success' ? 'check-circle' : notification.type === 'warning' ? 'exclamation-circle' : 'info-circle'} fa-2x"></i>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <div class="progress" style="height: 10px;">
                                <div class="progress-bar bg-success" style="width: ${performanceData.consistency}%"></div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="progress" style="height: 10px;">
                                <div class="progress-bar bg-info" style="width: ${performanceData.intensity}%"></div>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="progress" style="height: 10px;">
                                <div class="progress-bar bg-warning" style="width: ${performanceData.progress}%"></div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        recommendationHtml = `
            <div class="card h-100">
                ...
                <div class="card-body d-flex flex-column">
                    ...
                    <button class="btn btn-info w-100 mt-auto" data-bs-toggle="modal" data-bs-target="#exerciseDetailModal" data-id="${lastWorkout.id}" data-loggable="true">
        ...
        savedDietPlanHtml = `
            <div class="card h-100">
                ...
                <div class="card-body d-flex flex-column">
                    ...
                    <button class="btn btn-secondary w-100 mt-auto" data-page="diet">
...
function renderDietPlanPage() {
    ...
    if (currentUser.savedDietPlan) {
        return `
            ...
            <button class="btn btn-danger mt-4" id="clear-saved-plan-btn"><i class="fas fa-trash me-2"></i>Clear Saved Plan & Start New</button>
        `;
    }
    return `
...
function renderMealCard(meal, mealType) {
    ...
    const badgeClass = mealTypeColors[mealType] || 'bg-info';

    // Handle potential missing image
    const imageUrl = meal.image && meal.image.startsWith('images/recipes/') ? meal.image : 'images/diet/placeholder.JPG';
    const imageErrorScript = `this.onerror=null; this.src='images/diet/placeholder.JPG';`;

    // Handle preference badge
    const preferenceColors = {
        'Vegetarian': 'bg-success',
        'Vegan': 'bg-success-subtle text-success-emphasis',
        'Non-Vegetarian': 'bg-danger'
    };
            ...
            <div class="card h-100 diet-card shadow-sm">
                ...
                <div class="card-body d-flex flex-column">
                    ...
                    <div class="d-flex justify-content-around text-center my-3 macro-grid">
                        ...
                        <div>
                            <p class="mb-0 fs-5 fw-bold text-success">${meal.protein}g</p>
                            <small class="text-muted">Protein</small>
                        </div>
                        <div>
                            <p class="mb-0 fs-5 fw-bold text-warning">${meal.carbs}g</p>
                            <small class="text-muted">Carbs</small>
                        </div>
                        <div>
                            <p class="mb-0 fs-5 fw-bold text-danger">${meal.fats}g</p>
        ...
        if (workoutDetails) {
            activitiesHtml += `
                <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-2">
                    ...
                    <span class="badge bg-success">${workoutDetails.calories} cal</span>
                </div>
            `;
        }
    });
